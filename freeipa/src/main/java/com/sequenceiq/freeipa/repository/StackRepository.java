...
public interface StackRepository extends AccountAwareResourceRepository<Stack, Long>, JobResourceRepository<Stack, Long> {
    ...
    Optional<ResourceBasicView> findResourceCrnNameIdByNameAndAccountId(@Param("name") String name, @Param("accountId") String accountId);
    ...
    @Query("SELECT s.id as id, s.resourceCrn as resourceCrn, s.name as name " +
            ...
            "AND s.name in (:names) " +
            "AND s.accountId = :accountId")
    List<ResourceBasicView> findAllResourceCrnNameIdByNamesAndAccountId(@Param("names") Collection<String> names, @Param("accountId") String accountId);
